...
{
	return pow(val,4) - pow(val,2)*2.5;
}
...
{
	double exp_minus = exp(-pow(v-mu,2)/(2*pow(sigma,2)));
	double exp_plus = exp(-pow(v+mu,2)/(2*pow(sigma,2)));
	
	//return 0;
	return (1./pow(sigma,2))*(pow((v+mu)/sigma,2)*exp_plus + pow((v-mu)/sigma,2)*exp_minus - exp_plus - exp_minus);
}
