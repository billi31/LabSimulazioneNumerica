...
double FluidLJ::Acceptance(void){
	double acc = accepted/attempted;
	
	return acc;
}


void FluidLJ::Move(void){
	int o;
...
double FluidLJ::Boltzmann(double xx, double yy, double zz, int ip){
	double ene=0.0;
	double dx, dy, dz, dr;
...
void FluidLJ::InstantaneusUP(void){
	ofstream Epot, Pres;
...
void FluidLJ::Reset(void){ //Reset block averages
...
void FluidLJ::Accumulate(void){ //Update block averages
...
void FluidLJ::Averages(void){ //Print results for current block
	...
	err_press = Error(glob_av[iw],glob_av2[iw]);
	...
	for (int k=igofr; k<igofr+nbins; ++k){
		...
		if (iblk == nblk){
			Gave << setw(wd) << (k - igofr)*bin_size << setw(wd) << (double) glob_av[k]/nblk << setw(wd) << err_gdir << endl;
		}
	}
...
void FluidLJ::ConfXYZ(int nconf){ //Write configuration in .xyz format
	ofstream WriteXYZ;
...
double FluidLJ::Error(double sum, double sum2){
	if (iblk==1)
		return 0.0;
	else
...
double FluidLJ::Pbc(double r){  //Algorithm for periodic boundary conditions with side L=box
